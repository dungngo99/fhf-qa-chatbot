...
ParaphraseDict = {
    10: [
        'Is it necessary for my child to be evaluated for special education?',
    ...
    14: [
        'Is it possible for the school to use my confidential evaluation to speed up the review process?',
    ...
    18: [
        'How long will it take for my kid to be assessed?',
    ...
    20: [
        'What if the parent disagrees with the assessment?',
    ...
    23: [
        'Is it necessary for the school to inform me of who will be attending the meeting?',
    ...
    46: [
        'Are there age restrictions for different age groups in the same special education class?',
    ...
    51: [
        'At what age does a child typically receive special education services?',
    ...
    53: [
        'If my child was in Early Steps, will they go on to special education at their school?',
...
def find_similarities(df, nlp):
    results = {}

    for (correctIdx, questionList) in ParaphraseDict.items():
        correct_question = df.loc[correctIdx, 'questions']
        
        score = 0
        for var_question in questionList:
            var_doc = nlp(var_question)
            ranks = []

            for idx, orig_question in enumerate(df['questions'].values):
                if orig_question == "":
                    ranks.append((idx, 0))
                    continue

                orig_doc = nlp(orig_question)
                similarity = var_doc.similarity(orig_doc)
            ...
            bestIdx = sorted_ranks[0][0]

            if bestIdx == correctIdx:
                score += 1
    
        results[correct_question] = score/len(questionList)
        
    results['average score'] = sum(results.values()) / len(list(results.values())) 
    return results
...
def main():
    ...
    for model in models:
        ...
        results = find_similarities(df, nlp)
        comb_results[model] = results
        print(f"-----------Finished testing model {model}-----------")
        
    with open("app/data/output.json", 'w') as file:
        json.dump(comb_results, file)
